class Matriz {
    constructor(rows = 1, cols = 1) {
        if ((typeof rows !== 'number') && typeof cols !== 'number') throw new Error('Error en los argumentos');
        this.rows = rows;
        this.cols = cols;
        this.matrix = Array.from({length: rows}).fill(Array.from({length: cols}).fill(null));
    }

    setNewValues(m) {
        if (!(m instanceof Array)) throw new Error('No es matriz');
        const len = m[0].length;
        const valid = m.every(vec => (vec instanceof Array) && vec.length === len);
    ...
    fillRnd(max = 10) {
        this.matrix = this.matrix.map(arr => arr.map(val => Math.random() * max));
    }

    fillRange(max = 10) {
        this.matrix = this.matrix.map(arr => arr.map(() => Math.floor(Math.random() * max)));
    }
    ...
    sumM(m) {
        ...
        else if (!this.isValidForOperation('suma', m) || this.hasValidValues(this) || this.hasValidValues(m)) throw new Error('Matriz no compatible');

        let matrix = this.matrix.map((arr, y) => arr.map((e, x) => e + m.matrix[y][x]))
    ...
    subsM(m) {
        if (!(m instanceof Matriz)) throw new Error('Argumento no es Matriz')
        else if (!this.isValidForOperation('resta', m) || this.hasValidValues(this) || this.hasValidValues(m)) throw new Error('Matriz no compatible');
    ...
    multM(m) {
        if (!(m instanceof Matriz)) throw new Error('Argumento no es Matriz')
        else if (!this.isValidForOperation('mult', m) || this.hasValidValues(this) || this.hasValidValues(m)) throw new Error('Matriz no compatible');
    ...
    static matrizIdentidad(n = 2) {
        let x = 0;
        let matrix = Array.from({length: n}).fill(Array.from({length: n}).fill(null));
    ...
    isSimetrica() {
        ...
        return this.matrix.every((row, y) => row.every((val, x) => val === trans.matrix[y][x]));
    }
